...
/// Returns (eager_import, module_const) for a deferred type import.
///
/// Uses the "require the whole module" pattern to handle circular deps:
/// `const _m_Foo = require('./Foo');` — Node.js returns a reference to the
/// (initially empty) exports object that gets populated once the module
/// finishes loading.  Method bodies access `_m_Foo.Foo` at call time, when
/// the export is guaranteed to exist.
///
/// For interfaces: IID is also imported eagerly (needed at module load time
/// by registerInterface).
fn format_cjs_deferred_import(name: &str, kind: &str) -> (String, String) {
    let mod_const = format!("const _m_{name} = require('./{name}');\n");
    if kind == "interface" {
        let eager = format!("const {{ IID_{name} }} = require('./{name}');\n");
        // IID is eagerly destructured; module ref also stored for lazy wrapper access
        (eager, mod_const)
    } else {
        (String::new(), mod_const)
    }
}
...
pub fn generate_index(classes: &[ClassMeta], interfaces: &[InterfaceMeta], enums: &[TypeMeta]) -> String {
    ...
    for class in sorted_classes {
        ...
        let struct_names = collect_used_structs_from_class(class).iter()
    ...
    for iface in sorted_ifaces {
        ...
        let struct_names = collect_used_structs_from_iface(iface).iter()
            .flat_map(|s| js_struct_export_names(s))
            .collect::<Vec<_>>();
        if is_delegate {
            out.push_str(&format!(
            ...
            if struct_names.is_empty() {
                ...
                out.push_str(&format!(
                    "const {{ IID_{iname}, {iname}, {structs} }} = require('./{iname}');\n",
...
// ======================================================================
// Public: append_to_index — merge new types into an existing CJS index
// ======================================================================
...
pub fn append_to_index(
    ...
    // Parse existing exports from `module.exports = { ... };` line
    // and track which modules are already required
    let mut existing_exports: Vec<String> = Vec::new();
    let mut lines_without_exports: Vec<&str> = Vec::new();
    let mut required_modules: HashSet<String> = HashSet::new();

    for line in existing.lines() {
        if line.starts_with("module.exports") {
            // Extract names from `module.exports = { Foo, Bar, ... };`
            if let Some(start) = line.find('{') {
                if let Some(end) = line.rfind('}') {
                    ...
                    for name in names.split(',') {
                        let name = name.trim();
                        if !name.is_empty() {
                            existing_exports.push(name.to_string());
                        }
                    }
                }
            }
        } else {
            // Track module names from require('./Foo') lines
            if let Some(start) = line.find("require('./") {
                if let Some(end) = line[start + 11..].find("')") {
                    required_modules.insert(line[start + 11..start + 11 + end].to_string());
                }
            }
            lines_without_exports.push(line);
        }
    }

    let mut seen: HashSet<String> = existing_exports.iter().cloned().collect();
    let mut new_lines: Vec<String> = Vec::new();
    let mut new_exports: Vec<String> = Vec::new();

    // Add new classes
    let mut sorted_classes: Vec<_> = classes.iter().collect();
    sorted_classes.sort_by(|a, b| a.name.cmp(&b.name));
    for class in sorted_classes {
        if !required_modules.contains(&class.name) && seen.insert(class.name.clone()) {
            let struct_names = collect_used_structs_from_class(class).iter()
                .flat_map(|s| js_struct_export_names(s))
                .collect::<Vec<_>>();
            if struct_names.is_empty() {
                new_lines.push(format!("const {{ {} }} = require('./{}');", class.name, class.name));
            } else {
                new_lines.push(format!("const {{ {}, {} }} = require('./{}');", class.name, struct_names.join(", "), class.name));
                new_exports.extend(struct_names);
            }
            new_exports.push(class.name.clone());
        }
    }
    ...
    sorted_ifaces.sort_by(|a, b| a.name.cmp(&b.name));
    for iface in sorted_ifaces {
        if required_modules.contains(&iface.name) || !seen.insert(iface.name.clone()) { continue; }
        let is_delegate = iface.methods.iter().any(|m| m.name == ".ctor")
            && iface.methods.iter().any(|m| m.name == "Invoke");
        let struct_names = collect_used_structs_from_iface(iface).iter()
            .flat_map(|s| js_struct_export_names(s))
        ...
        if is_delegate {
            new_lines.push(format!(
                "const {{ IID_{0}, {0}_PARAM_TYPES }} = require('./{0}');",
                iface.name
            ));
            new_exports.push(format!("IID_{}", iface.name));
            new_exports.push(format!("{}_PARAM_TYPES", iface.name));
        } else {
            if struct_names.is_empty() {
                new_lines.push(format!(
                    "const {{ IID_{0}, {0} }} = require('./{0}');",
                    iface.name
                ));
            } else {
                new_lines.push(format!(
                    "const {{ IID_{0}, {0}, {1} }} = require('./{0}');",
                    iface.name, struct_names.join(", ")
                ));
                new_exports.extend(struct_names);
            }
            new_exports.push(format!("IID_{}", iface.name));
            new_exports.push(iface.name.clone());
        }
    }
    ...
    for en in sorted_enums {
        if let TypeMeta::Enum { name, .. } = en {
            if !required_modules.contains(name) && seen.insert(name.clone()) {
                new_lines.push(format!("const {{ {} }} = require('./{}');", name, name));
                new_exports.push(name.clone());
            }
        }
    }

    // Rebuild: existing lines + new lines + merged module.exports
    let mut out = String::new();
    for line in &lines_without_exports {
        out.push_str(line);
        out.push('\n');
    }
    ...
    // Merge all exports (existing + new), sorted
    existing_exports.extend(new_exports);
    existing_exports.sort();
    existing_exports.dedup();
    out.push_str(&format!("module.exports = {{ {} }};\n", existing_exports.join(", ")));
    out
}
...
pub fn generate_interface(iface: &InterfaceMeta, known_types: &HashSet<String>, delegate_type_names: &HashSet<String>) -> String {
    ...
    // Import parameterized collection types (module ref to avoid circular deps)
    ...
    for cname in &collection_names {
        if cname != &iface.name && !delegate_names.contains(cname) {
            out.push_str(&format!("const _m_{0} = require('./{0}');\n", cname));
            deferred_names.insert(cname.clone());
        }
    }

    // Import delegate IID + PARAM_TYPES for event methods (eager — delegates don't form cycles)
    ...
    // Type imports for referenced types (module ref to avoid circular deps)
    ...
    for (_ns, name, kind) in &sorted_type_imports {
        if known_types.contains(name) && !delegate_names.contains(name) {
            let (eager, mod_const) = format_cjs_deferred_import(name, kind);
            if !eager.is_empty() { out.push_str(&eager); }
    ...
    for method in &iface.methods {
        out.push('\n');
        out.push_str(&generate_js_iface_instance_method(iface, &iface_var, method, known_types, delegate_type_names, &deferred_names));
    }

    out.push_str("}\n");
    exports.push(iface.name.clone());

    // Export struct helpers
    for s in &used_structs {
        exports.extend(js_struct_export_names(s));
    }
...
pub fn generate_class(class: &ClassMeta, known_types: &HashSet<String>, delegate_type_names: &HashSet<String>, shared_iids: &HashSet<String>) -> String {
    ...
    // Collection generics import (module ref to avoid circular deps)
    ...
    // Type imports — only for types that have generated files (module ref to avoid circular deps)
    ...
    for iface in &class.static_interfaces {
        for method in &iface.methods {
            out.push('\n');
            out.push_str(&generate_js_static_method_invoke(class, iface, method, known_types, &deferred_names));
        }
    }
    ...
    for req_iface in &class.required_interfaces {
        ...
        if imported_names.contains(&req_iface.name) {
            // Don't re-export deferred imports — they are available from their own module
            if !deferred_names.contains(&req_iface.name) {
        ...
        for method in &req_iface.methods {
            out.push('\n');
            out.push_str(&generate_js_iface_instance_method(req_iface, &reg_var, method, known_types, delegate_type_names, &deferred_names));
        }
        out.push_str("}\n");
    }
...
fn generate_js_struct_helpers(s: &TypeMeta) -> String {
    ...
    out.push_str(&format!("function unpack{}(v) {{\n", name));
...
/// Returns the exported names for struct helpers of a given struct type.
...
fn generate_js_static_method_invoke(
    ...
    if method.is_property_getter && in_params.is_empty() {
        ...
        let converted = convert_return(&invoke_expr, return_type, false, known_types, deferred_names);
...
fn generate_js_iface_instance_method(
    ...
    generate_js_method_body(iface_var, "this._obj", method, known_types, delegate_type_names, deferred_names, None)
}
...
fn generate_js_method_body(
    ...
    if method.is_property_getter && in_params.is_empty() {
        ...
        let converted = if is_delegate_type(return_type) {
            ...
            convert_return(&invoke_expr, return_type, false, known_types, deferred_names)
        };
        out.push_str(&format!("        return {};\n", converted));
        out.push_str("    }\n");
        ...
        if is_with_progress {
            ...
            let converted = convert_array_return(&arr_expr, elem, known_types, deferred_names);
            out.push_str(&format!("        return {};\n", converted));
        } else {
            let converted = convert_return(&invoke_expr, return_type, is_async, known_types, deferred_names);
            out.push_str(&format!("        return {};\n", converted));
