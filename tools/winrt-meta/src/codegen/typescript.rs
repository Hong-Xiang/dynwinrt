...
pub fn generate_class(class: &ClassMeta, known_types: &HashSet<String>, delegate_type_names: &HashSet<String>) -> String {
    ...
    if let Some(ref default_iface) = class.default_interface {
        for method in &default_iface.methods {
            out.push('\n');
            out.push_str(&generate_instance_method_invoke(default_iface, method, known_types, &delegate_type_names));
        }
    }
...
fn generate_iface_instance_method(
    ...
    generate_method_body(iface_var, "this._obj", method, known_types, delegate_type_names)
}
...
fn generate_instance_method_invoke(
    ...
    generate_method_body(&default_iface.name, &cast_expr, method, known_types, delegate_type_names)
}
...
fn generate_method_body(
    ...
    if method.is_property_getter && in_params.is_empty() {
        ...
        let converted = if is_delegate_type(return_type) {
            ...
            convert_return(&invoke_expr, return_type, false)
        };
        out.push_str(&format!("        return {};\n", converted));
        out.push_str("    }\n");
...
fn wrap_arg(name: &str, typ: &TypeMeta) -> String {
    match typ {
        ...
        // Struct, Guid, etc. — pass as-is (caller provides DynWinRtValue)
        _ => name.to_string(),
    }
}
