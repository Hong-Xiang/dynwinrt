    ...
    for iface in sorted_ifaces {
        ...
        if is_delegate {
            ...
            if struct_names.is_empty() {
                ...
                out.push_str(&format!(
                    "export {{ IID_{iname}, {iname}, {structs} }} from './{iname}';\n",
...
pub fn append_to_index(
    ...
    for line in existing.lines() {
        ...
        if line.starts_with("export {") {
            ...
            // Track module name from `from './Foo'`
    ...
    for iface in sorted_ifaces {
        ...
        if is_delegate {
            out.push_str(&format!(
                "export {{ IID_{0}, {0}_PARAM_TYPES }} from './{0}';\n",
                iface.name
            ));
        } else {
            if struct_names.is_empty() {
                out.push_str(&format!(
                    "export {{ IID_{0}, {0} }} from './{0}';\n",
                    iface.name
                ));
            } else {
                out.push_str(&format!(
                    "export {{ IID_{0}, {0}, {1} }} from './{0}';\n",
